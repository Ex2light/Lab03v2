import os


class TicTacToe:
    def __init__(self) -> object:
        self.board = [['*', '*', '*'] for x in range(3)]
        self.End = False
        self.xWin = False
        self.moveCounter = 0

    def printBoard(self):
        for i in self.board:
            print(i[0] + ' ' + i[1] + ' ' + i[2])

    def movePlayer1(self):
        index = int(input("Player 1 turn. Type next position  "))
        if index > 9:
            print("Spot out of range, select other")
            self.movePlayer1()

        elif self.board[(index - 1) // 3][(index % 3) - 1] == '*':
            self.board[(index - 1) // 3][(index % 3) - 1] = 'X'
            self.moveCounter += 1
        else:
            if self.board[(index - 1) // 3][(index % 3) - 1] == 'X':
                print("Spot ocupied by You, select other")
                self.movePlayer1()
            elif self.board[(index - 1) // 3][(index % 3) - 1] == 'O':
                print("Spot ocupied by Player 2, select other")
                self.movePlayer1()


        self.isEnd()

    def movePlayer2(self):
        index = int(input("Player 2 turn. Type next position  "))
        if index > 9:
            print("Spot out of range, select other")
            self.movePlayer2()

        elif self.board[(index - 1) // 3][(index % 3) - 1] == '*':
            self.board[(index - 1) // 3][(index % 3) - 1] = 'O'
            self.moveCounter += 1

        else:
            if self.board[(index - 1) // 3][(index % 3) - 1] == 'X':
                print("Spot ocupied by Player 1, select other")
                self.movePlayer2()
            elif self.board[(index - 1) // 3][(index % 3) - 1] == 'O':
                print("Spot ocupied by You, select other")
                self.movePlayer2()

            
        self.isEnd()

    def game(self):
        #os.system("clear")
        print(
            "Welcome to the tic-tac-toe game.\nFirst player is 'X' and second 'O'.\nTo enter your moves type numbers 1-9")
        self.printBoard()
        while not self.End:
            self.movePlayer1()
            #os.system("clear")
            self.printBoard()
            if not self.End:
                if self.moveCounter == 9:
                    break

                self.movePlayer2()
                #os.system("clear")
                self.printBoard()
        if self.xWin:
            print("Player 1 has won")
        elif self.End:
            print("Player 2 has won")
        else:
            print("Draw")

    def isEnd(self):
        for i in range(3):
            if self.board[i] == ['X', 'X', 'X']:
                self.xWin = True
                self.End = True
            if self.board[i] == ['O', 'O', 'O']:
                self.End = True
            for j in range(3):
                if self.board[j][i] != 'X':
                    break
            else:
                self.xWin = True
                self.End = True

            for j in range(3):
                if self.board[j][i] != 'O':
                    break
            else:
                self.End = True
        for k in range(3):
            if self.board[k][k] != 'X':
                break
        else:
            self.End = True
            self.xWin = True
        for k in range(3):
            if self.board[k][k] != 'O':
                break
        else:
            self.End = True

        if self.board[1][1] == self.board[0][2] == self.board[2][0] != '*':
            self.End = True
            if self.board[1][1] == 'X':
                self.xWin = True
